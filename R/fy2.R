...
#' @param fy_start_month a number represeting the starting month for the fiscal year (defaults to 7 -- "July")
#'
...
fy2  <- function(date,fy_start_month=7) {
  ...
  if(!is.character(date)&!lubridate::is.Date(date)&!lubridate::is.POSIXct(date)&!lubridate::is.POSIXlt(date)) {
    stop("Please use values formatted as dates or charcater strings with only numbers convertible to dates like '2017-07-01'")
  }
